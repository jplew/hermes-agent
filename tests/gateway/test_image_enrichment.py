...
@pytest.mark.asyncio
async def test_image_enrichment_surfaces_gateway_url_as_noncanonical_metadata():
    from gateway.run import GatewayRunner
    ...
    with patch(
        ...
        enriched = await runner._enrich_message_with_vision(
            "",
            ["/tmp/test.jpg"],
            image_cdn_urls=["https://media.example.com/test.jpg"],
        )

    assert "Gateway media URL available for reference" in enriched
    assert "https://media.example.com/test.jpg" in enriched
    assert "canonical imageUrl" in enriched
